...
class Book(IdObject):
  ...
  def get_items(self):
    return f"{self._book_id},{self._author},{self._title},{self.__rented} \n"
    
  def __str__(self) -> str:
    return f"[{self._book_id}] Author: {self._author} | Title: {self._title} | {self.__rented}"
...
class Rental(IdObject):
  ...
  def __init__(self,rental_id,client:IdObject,book:IdObject,rental_date:date,returned_date:date) -> None:
    ...
    self._rental_date_file = rental_date
    self._returned_date_file = returned_date
  ...
  def get_items(self):
    return f"{self.__rental},{self._client_id.id},{self._book.id},{self._rental_date_file},{self._returned_date_file} \n"
